...
class PV:
    ...
    def __str__(self):
        ...
        print(self.name, 'PV\n')
        for attr in attrs:
            print(f"{' '.join(attr[0].split('_')).title()}: {attr[1]}") if attr[0] not in ['READID', 'SETID'] else print(f"{' '.join(attr[0].split('_'))}: {attr[1]}")
        print('==' * 30)
        return ''
    ...
    try:
        for i in range(num_inputs):
            ...
            for k in range(repeats):
                ...
                true_values[i, k] = v[k]
            print(f"\rStep %d/%d complete!" % (i + 1, num_inputs), end = '', flush = True) if len(inputs) > 1 else None
    except:
        print('\nAn error caused the script to crash.')
        pass

    for pv in PVs:
        caput(pv.SETID, pv.default)
...
def WrapSingleInputFunction(f):
    return lambda x, numRepeats = 1, s = 0: f(x * jnp.ones(numRepeats)) + s * np.random.randn(numRepeats)

def EvaluateFunctionAtInputs(x, f, numRepeats = 1, s = 0):
    return jnp.array([jnp.array([objective(x, numRepeats = numRepeats, s = s) for objective in gp.VOCS['objectives'].values()]) for x in x])